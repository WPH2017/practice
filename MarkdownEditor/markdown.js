...
//    节点插入
    let changeText=function (string) {
        // string.replace(/#.+/g,function (sub,$1,idx,string) {
        //     console.log(sub);
        // });
        let h_pattern=new RegExp(/#.+/,'g');
        let ol_pattern=new RegExp(/-.+\n/,'g');
        var temp=1;
        var result=[];
        while (temp){
            result.push(temp);
            temp=h_pattern.exec(string)||h_pattern.exec(string);
        }
        return result;
    };
    let html=`
...
- BFC的几个生成条件：
    ...
    `;
    let textArray=changeText(html);
    textArray.shift();
    console.log(textArray)
    textArray.forEach(function (val,idx) {
        if(/#\s\w/.test(val[0])){
            textArray[idx]=`<p class="h1">${val[0]}</p>`;
        }else if(/##\s\w/.test(val[0])){
            textArray[idx]=`<p class="h2">${val[0]}</p>`;
        }else if(/###\s\w/.test(val[0])){
            textArray[idx]=`<p class="h3">${val[0]}</p>`;
        }else if(/-\s\w/gm.test(val[0])) {
            textArray[idx] = `<li class="li">${val[0]}</li>`;
        }
    });
    var paragraph=document.createElement('div');
    paragraph.innerHTML=textArray.join('');
